...
class MyForm(QtGui.QMainWindow):
	...
	def pullShotgunData(self) : 
		...
		for shotName in mayaShots : 
			status = sgShots[shotName]['sg_status_list']
			if not shotName in sgShots.keys() : 
				removeShots.append(shotName)

			if shotName in sgShots.keys() : 
				if status == 'omt' : 
					removeShots.append(shotName)
	...
	def buildSequencerShot(self, buildSource) : 
		...
		if buildShotInfo : 
			...
			self.completeDialog('Error', 'No data info')
	...
	def setAutoShotName(self) : 
		...
		if self.ui.auto_checkBox.isChecked() : 
			...
			if self.shotInfo : 
				...
				# get the highest shot number
				allShotNum = []
				maxShotNum = 0
				maxRow = None
				for i in range(allRow) : 
					shotName = self.getIndexWidgetItem(i)[0]
					shotNum = re.findall('\d+', shotName)[0]
				...
				lastShot = self.getIndexWidgetItem(maxRow)[0]
